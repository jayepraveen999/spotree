...
\begin{tikzpicture}
  ...
  \node at (0,0.4) {\includegraphics[width=3.2cm]{jayendra.jpeg}};
\end{tikzpicture}
...
\vspace{4pt}
{\small Applied research in Earth observation, MLOps and real-time inference pipelines for satellite-based wildfire detection. Worked on Tree Species classification using satellite imagery at TUM. Presented at \textbf{ESA Living Planet Symposium 2025} and \textbf{ECAI 2020}.}

\vspace{6pt}
{\small\textbf{Focus areas:} Geospatial Data Engineering, Active fire detection using satellite imagery, Deep Learning for semantic segmentation, MLOps, CI/CD.}
...
\begin{tikzpicture}
  \clip (0,0) circle (1.6cm);
  \node at (0,0) {\includegraphics[width=3.2cm]{hari.jpeg}};
\end{tikzpicture}
...
\clearpage
\subsection{Login}
Students authenticate via email and password using Supabase Auth. New users can create an account through the Sign Up flow, where they also select their school affiliation.

\vspace{1em}
\begin{center}
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/login_page.PNG}\\
{\small\textit{The Spotree login screen --- ``Spot trees. Drop beats. Explore Munich.'' Students sign in with email and password.}}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/profile_2.jpg}\\
{\small\textit{The Sign Up form: students enter their name, email, password, and optionally their school to help track which schools are most active on Spotree.}}
\end{minipage}
\end{center}
...
\begin{center}
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/map_1.PNG}\\
{\small\textit{Overview of Munich showing all spotted trees. Leaf markers indicate tree locations; markers with a music badge have a linked Spotify song.}}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/map_2.PNG}\\
{\small\textit{Tapping a marker opens a detail card with species, health, height, mapper info, and a play button for the linked song (here: ``Shape of You'').}}
\end{minipage}
\end{center}
...
\begin{itemize}
  ...
  \item Rate \textbf{health status}: Healthy, Minor issues, Moderate damage, Severe damage, Dead/Dying
  \item Estimate \textbf{tree height}: Under 5m (small/young), 5--10m (medium), 10--20m (tall), 20--30m (very tall), Over 30m (giant)
  \item Estimate \textbf{trunk diameter}: Under 10cm (sapling), 10--30cm (young), 30--60cm (mature), 60--100cm (old), Over 100cm (ancient)
...
\includegraphics[height=7.5cm]{../app_screenshots/capture_1.PNG}\\
{\small\textit{The capture form with Take Photo and Gallery options. Location coordinates will be fetched automatically from the device's GPS or GPS coordinates tagged in the photo's EXIF data.}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_2.PNG}\\
{\small\textit{The Tree species dropdown shows Munich's native tree species with scientific names and photos. An option for ``Not Sure'' allows students to proceed without guessing.}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_3.PNG}\\
{\small\textit{Health status dropdown with five assessment levels from Healthy to Dead/Dying, allowing students to evaluate the tree's condition.}}
\end{minipage}
\end{center}

\begin{center}
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_4.PNG}\\
{\small\textit{Estimated height dropdown with five categories from Under 5m (small/young) to Over 30m (giant).}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_5.PNG}\\
{\small\textit{Trunk diameter dropdown with size/age categories from Under 10cm (sapling) to Over 100cm (ancient).}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_6.PNG}\\
{\small\textit{Drop a Song section at the bottom of the form, where students can paste a Spotify track URL and name the song to link it with the tree.}}
\end{minipage}
\end{center}
...
\textbf{Example: Spotting a Tree}

\vspace{1em}
\begin{center}
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_7.PNG}\\
{\small\textit{A photo with tree is uploaded. After a photo is uploaded, student can click on ``Add Tree'' button for AI validation. ``Checking for trees\ldots'' appears while the image is analyzed by LLaVA 1.5.}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_8.PNG}\\
{\small\textit{After a photo is uploaded, the AI validates it. ``Checking for trees\ldots'' appears while the image is analyzed by LLaVA 1.5}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_9.PNG}\\
{\small\textit{Validation passed ``Tree verified'' badge displayed with extracted GPS coordinates. The form fields below are now active for data entry.}}
\end{minipage}
\end{center}

\begin{center}
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_10.PNG}\\
{\small\textit{ Full form filled out with species selected, health status, height and diameter estimates and confidence levels}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_11.PNG}\\
{\small\textit{Example Spotify song added to the tree, with the URL and display name fields filled out.}}
\end{minipage}\hfill
\begin{minipage}{0.30\textwidth}\centering
\includegraphics[height=7.5cm]{../app_screenshots/capture_12.PNG}\\
{\small\textit{``Tree spotted!'' --- the success dialog confirms the tree has been added to the map, with an option to view it directly.}}
\end{minipage}
\end{center}
...
\begin{center}
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/explore_1.PNG}\\
{\small\textit{The Explore feed shows recently spotted trees with photos, species, health status, confidence bars, linked songs with play buttons, and a ``View on Map'' button.}}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/explore_2.PNG}\\
{\small\textit{Scrolling reveals more entries from different users and schools, each with their own tree data and song choice.}}
\end{minipage}
\end{center}
...
\begin{center}
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/guide_1.PNG}\\
{\small\textit{The Spotree Guide presents Munich's native tree species in a visual grid. Each card shows a representative photo, common name, and scientific name.}}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}\centering
\includegraphics[height=9cm]{../app_screenshots/guide_3.PNG}\\
{\small\textit{Species detail view for European Beech: plant description chips, growth requirements, natural habitat, common uses, and a link to Easyscape for deeper learning.}}
\end{minipage}
\end{center}
...
\vspace{1em}
\begin{center}
\includegraphics[height=9cm]{../app_screenshots/profile.PNG}\\
{\small\textit{The Profile tab shows the student's name, email, school affiliation, and stats --- Trees Spotted, Songs Shared, and Species Found. Below, a list of all personally mapped trees with dates, health status, and song indicators.}}
\end{center}
...
\vspace{1em}
\begin{center}
\includegraphics[width=\textwidth]{../app_screenshots/admin_dashboard_screenshot.png}\\
{\small\textit{The admin dashboard showing live stats (18 trees, 3 users, 16 species, 14 songs), species breakdown with distribution bars, school-level analytics (LMU vs.\ TUM), health overview, real-time activity feed, and an interactive dark-themed map of all spotted trees across Munich.}}
\end{center}
...
% ══════════════════════════════════════════════════════════════
\clearpage
\section{Local Deployment \& Testing}

The Spotree prototype is fully open-source and can be cloned, built, and tested on any machine with Node.js installed. The following steps allow reviewers to run the app locally on their own mobile device.

\textbf{Prerequisites:}
\begin{itemize}
  \item \textbf{Node.js} (v18 or later) --- \url{https://nodejs.org}
  \item \textbf{Git} --- to clone the repository
  \item \textbf{Expo Go} app installed on your mobile device (available on both the \href{https://apps.apple.com/app/expo-go/id982107779}{App Store} and \href{https://play.google.com/store/apps/details?id=host.exp.exponent}{Google Play})
  \item Your computer and mobile device must be on the \textbf{same Wi-Fi network}
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{verbatim}
  git clone https://github.com/jayepraveen999/spotree.git
  cd spotree
  \end{verbatim}
  ...
  \item \textbf{Start the Expo development server:}
  \begin{verbatim}
  npx expo start
  \end{verbatim}

  \item \textbf{Scan the QR code} that appears in the terminal:
  \begin{itemize}
    \item \textbf{iOS:} Open the default Camera app and point it at the QR code. Tap the Expo Go banner that appears.
    \item \textbf{Android:} Open the Expo Go app and tap ``Scan QR Code.''
  \end{itemize}

  \item The app will bundle and load on your device. You can now \textbf{create an account}, explore the map, spot trees, and test all features.
\end{enumerate}

\textbf{Admin Dashboard:} To view the admin analytics dashboard, open the file \texttt{admin/index.html} in any web browser. It connects directly to the same Supabase database and displays live data --- no server setup required.

\textbf{Source Code:} \url{https://github.com/jayepraveen999/spotree}

\begin{center}
{\small If you have any questions or would like to discuss the proposal further, please don't hesitate to reach out to us. We look forward to the opportunity to bring Spotree to life for Munich's students.}

\vspace{0.5cm}
{\small\bfseries Thank you for your consideration.}
\end{center}
